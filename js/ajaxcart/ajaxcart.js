...
AjaxcartTools.prototype = {
	initialize: function(){
		this.browserTypeIE = jQueryAC.browser.msie;
		...
		if ($(id+'-popup-container').style.position=='absolute' || $(id+'-popup-container').style.position=='fixed'){
			...
			if (id!='ajaxcart-loading') {		
				...
				if (ajaxcart.showNotificationBkg == 1 && ajaxcartTools.browserTypeIE && ajaxcartTools.browserVersion <= 8) {					
			    	...
			    	jQueryAC('#ac-popup-wrapper-bkg').show();
			    	jQueryAC('#ac-popup-wrapper-bkg').animate({'backgroundColor': 'rgba(' + ajaxcart.notificationWrapperBkg + ', 0.6)'},300);	
				}
			}
		}
	},
...
Ajaxcart.prototype = {
	...
	updateAjaxCartBlocks: function(response){		
		...
		jQueryAC('button').livequery(function(){ 	
			ajaxcart.updateButton(this);
		})	
		//update the parent window as well if actions are performed from the compare popup
		if (window.opener != null) {
			jQueryAC('button', ajaxcart.updateWindow.document).livequery(function(){ 	
				ajaxcart.updateButton(this);
			})		
		}
				    
		//Change cart/wishlist/compare links + add product id attribute to images, used for drag and drop
		jQueryAC('a').livequery(function(){ 	
			ajaxcart.updateLink(this);
		})			
		//update the parent window as well if actions are performed from the compare popup
		if (window.opener != null) {
			jQueryAC('a', ajaxcart.updateWindow.document).livequery(function(){ 	
				ajaxcart.updateLink(this);
			})		
		}
			
		//UPDATE CATEGORY PAGE
		jQueryAC('.' + ajaxcart.categoryList + ' li').livequery(function(){ 					
			var li = this;
			    	...
			    	if ( onclickCartAction.indexOf("checkout/cart/add") != -1 || onclickCartAction.indexOf("ajaxcart/index/init") != -1) {
			    		...
			    		if (ajaxcart.dragdropCategory == 1){
							jQueryAC(li).find('img').each(function() {
								var src = jQueryAC(this).attr('src');
					            if (!jQueryAC(this).parent().attr("product") && src != '' && src != null && src.indexOf('media/catalog/product/')!=-1){
					    			jQueryAC(this).wrap('<div></div>');
					        		jQueryAC(this).parent().attr("product",productId);	
					        	}
							});
			        	}
			    	}
		...
		//UPDATE PRODUCT PAGE
		jQueryAC('#product_addtocart_form').livequery(function(){ 
			//Change add to cart button		
		...
		//UPDATE CART SIDEBAR
		jQueryAC('.'+ajaxcart.cartSidebar +' div', ajaxcart.updateWindow.document).livequery(function(){	
			for (var i=0;i<=ajaxcart.lastCartSidebar;i++) {
				//add tooltip
				if (i==ajaxcart.lastCartSidebar && ajaxcart.enableTooltip != 0 && ajaxcart.dragdropCategory == 1 && !jQueryAC('#'+ajaxcart.cartSidebar + ajaxcart.lastCartSidebar + ' span.tooltip-sidebar', ajaxcart.updateWindow.document).length > 0){		
		...
		//UPDATE CART PAGE
		jQueryAC('#'+ajaxcart.cartPage+' input', ajaxcart.updateWindow.document).livequery(function(){	
		...
		jQueryAC('.'+ajaxcart.wishlistSidebar +' div', ajaxcart.updateWindow.document).livequery(function(){	
			for (var i=0;i<=ajaxcart.lastWishlistSidebar;i++) {
				//add tooltip
				if (i==ajaxcart.lastWishlistSidebar && ajaxcart.enableTooltip != 0 && ajaxcart.dragdropCategory == 1 && !jQueryAC('#'+ajaxcart.wishlistSidebar + ajaxcart.lastWishlistSidebar +' span.tooltip-sidebar', ajaxcart.updateWindow.document).length > 0){		
		...
		jQueryAC('#'+ajaxcart.wishlistPage+' input', ajaxcart.updateWindow.document).livequery(function(){	
			//add increase/decrease buttons to qty inputs
			var input = jQueryAC(this);
		...
		jQueryAC('.'+ajaxcart.compareSidebar +' div', ajaxcart.updateWindow.document).livequery(function(){	
			for (var i=0;i<=ajaxcart.lastCompareSidebar;i++) {
				//add tooltip
				if (i==ajaxcart.lastCompareSidebar && ajaxcart.enableTooltip != 0 && ajaxcart.dragdropCategory == 1 && !jQueryAC('#'+ajaxcart.compareSidebar + ajaxcart.lastCompareSidebar + ' span.tooltip-sidebar', ajaxcart.updateWindow.document).length > 0){		
	...
	updateButton: function(button) {
	    ...
	    } else if ($('ac-cart') && jQueryAC(button).hasClass('btn-update')) {
	    	//change cart page update cart button
		    jQueryAC(button).click(function( event ) {
			    event.preventDefault();
			    ajaxcart.updateCartQty('',false);
			});
	    } else if ($('ac-wishlist') && jQueryAC(button).attr('name')=='do') {
	    	//change cart page update cart button
		    jQueryAC(button).click(function( event ) {
			    event.preventDefault();
				ajaxcart.updateWishlistItems();
				    	...
				    	start: function(event, ui) {
				    		...
				    		if (ajaxcart.enableTooltip){	
								...
								if (ajaxcartTools.browserTypeIE && ajaxcartTools.browserVersion <= 8){
				    				...
				    				jQueryAC('span.tooltip-sidebar').animate({opacity: "show", top: "-35"}, "fast");
				    			}
				    		}
				    		
					    	jQueryAC('.ui-draggable-dragging').css({'z-index':'9999'});
							...
							if (ajaxcartTools.browserTypeIE && ajaxcartTools.browserVersion <= 9){
					    		...
					    		jQueryAC('.ui-draggable-dragging').addClass('mouse-over');
					    	}
	...
	makeWishlistDroppableArea: function() { 
	    jQueryAC('#'+ajaxcart.wishlistSidebar + ajaxcart.lastWishlistSidebar).droppable({
	    	accept: function() { return true; },
	    	over: function( event, ui ) {
		...
		if (isPopup || (!url && $('product_addtocart_form'))){
			...
			if (!this.ajaxCartRunning) {
				...
				if (!ajaxcart.itemDropped) {
					var productImageContainer = jQueryAC('div[product="'+productId+'"]');
					productImageContainer.removeAttr('style');
					var clone = productImageContainer.clone().appendTo(productImageContainer.parent());
					var cartSidebar = jQueryAC('#'+ajaxcart.cartSidebar + ajaxcart.lastCartSidebar);
					...
					setTimeout(function() {						    	
					    ...
					    clone.animate(
							{crSpline: jQueryAC.crSpline.buildSequence([[0, 0], [cartX/4, cartY-150], [cartX/4*2, cartY-250], [cartX/4*3, cartY-150], [cartX, cartY]])}, 
							800,
							function() {
								clone.remove();
			...
			for (var i=0;i<response.update_section.html_cart.length;i++) {
				if (response.update_section.html_cart[i] && this.updateWindow.document.getElementById(ajaxcart.cartSidebar+i)) {
			    	...
			    	if (window.opener != null && window.location.href.indexOf('catalog/product_compare/index/')!=-1 && $(ajaxcart.comparePopup)) {
					    jQueryAC(cartSidebar).hide().delay(15).fadeIn(300);
					} else {
			...
			if (!cartLink) {
	    		jQueryAC('#ac-links a', this.updateWindow.document).each(function() {
					...
					if (href != null && href != '' && href.indexOf('/checkout/cart/') != -1) {
						jQueryAC(this).html(response.update_section.html_cart_link);
					}    			
				});
			} else {
				jQueryAC(cartLink).html(response.update_section.html_cart_link);
			}
    	}
	},	
	...
	addWishlistItemToCart: function(url, button){ 
		if (!this.ajaxCartRunning) {			
			...
			//if wishlist page, add param to load new wishlist page html as well	
			if (jQueryAC(('#' + ajaxcart.wishlistPage), ajaxcart.updateWindow.document).length > 0 && !$$('#'+formId+' #is_wishlist_page')[0]) {
	...
	updateWishlist: function(response){	
		...
		*/
			    	...
			    	if (window.opener != null && window.location.href.indexOf('catalog/product_compare/index/')!=-1 && $(ajaxcart.comparePopup)) {
					    jQueryAC(wishlistSidebar).hide().delay(15).fadeIn(300);
					} else {
		...
		if (response.update_section.html_compare) {
			for (var i=0;i<response.update_section.html_compare.length;i++) {
				if (response.update_section.html_compare[i] && this.updateWindow.document.getElementById(ajaxcart.compareSidebar+i)) {
			    	...
			    	if (window.opener != null && window.location.href.indexOf('catalog/product_compare/index/')!=-1 && $(ajaxcart.comparePopup)) {
					    jQueryAC(compareSidebar).hide();
					    setTimeout(function() {jQueryAC(compareSidebar).fadeIn(300);}, 30);
					} else {
