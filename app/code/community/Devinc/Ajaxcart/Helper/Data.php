		...
		if($this->isEnabled() && Mage::getSingleton('customer/session')->getAjaxCartAction()=='cart-sidebar'){
			...
			if (Mage::getStoreConfig('ajaxcart/qty_configuration/show_qty_in_cartsidebar')){
				...
				if (Mage::getStoreConfig('ajaxcart/qty_configuration/qty_buttons_in_cartsidebar')){
					$html = '<span class="ajaxcart-qty" id="sidebar-qty-container-'.$itemId.'"><input name="cart['.$itemId.'][qty]" id="sidebar-qty-'.$itemId.'" type="text" value="'.$_qty.'" class="input-text qty ajaxcart-qty-input" /><span class="qty-control-box"><button class="increase" href="javascript:void(0)" onclick="ajaxcart.qtyUp(\''.$productId.'\',\'sidebar-qty-'.$itemId.'\',this);"><span>+</span></button><button class="decrease" href="javascript:void(0)" onclick="ajaxcart.qtyDown(\''.$productId.'\',\'sidebar-qty-'.$itemId.'\',this);"><span>-</span></button></span></span>';
				} else {
					$html = '<span class="ajaxcart-qty" id="sidebar-qty-container-'.$itemId.'"><input name="cart['.$itemId.'][qty]" id="sidebar-qty-'.$itemId.'" type="text" value="'.$_qty.'" class="input-text qty ajaxcart-qty-input" /></span>';
				}
				return $html;
			} else {
